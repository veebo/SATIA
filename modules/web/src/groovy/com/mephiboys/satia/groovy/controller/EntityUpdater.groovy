...
public class EntityUpdater {
	...
	def newPhrase(String newValue, Lang lang) {
        Phrase equalPhrase = ks.getEntityByQuery(Phrase.getClass(),"SELECT * FROM phrases WHERE value=?", newValue);
        if (equalPhrase != null) {
            return equalPhrase;
        }
        else {
    ...
    def newTask(String[] values, Generator gen, Test test) {
        ...
        Translation tr = ks.getEntityByQuery(Translation.getClass(),
            "SELECT * FROM translations WHERE (phrase1_id=? AND phrase2_id=?) OR (phrase1_id=? AND phrase2_id=?)",
    ...
    def removeTask(Task t, Test test) {
    	...
    	Collection<Task> relTasks = ks.getEntitiesByQuery(Task.getClass(),
    		"SELECT * FROM tasks WHERE translation_id=?", tr.getTranslationId());
    	if (relTasks().isEmpty()) {
    		ks.deleteEntityById(Translation.getClass(), tr.getTranslationId());
    	}
    	Collection<Translation> relTranlations1 = ks.getEntitiesByQuery(Translation.getClass(),
    		"SELECT * FROM translations WHERE phrase1_id=? OR phrase2_id=?",
    		p1.getPhraseId(), p1.getPhraseId());
    	if (relTranlations1.isEmpty()) {
    		ks.deleteEntityById(Phrase.getClass(), p1.getPhraseId());
    	}
    	Collection<Translation> relTranlations2 = ks.getEntitiesByQuery(Translation.getClass(),
    		"SELECT * FROM translations WHERE phrase1_id=? OR phrase2_id=?",
